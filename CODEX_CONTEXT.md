...
>
> 문서 내용의 앱 기능 기준 SHA: `bcd1667dd295b94ba7eb8fef409ae135cee729ee`
>
...
- 2026-08-08 운영 배포 검증에서 운영 URL, `404.html`, `manifest.webmanifest`, `sw.js`와 배포 HTML·service worker·main JS에서 동적으로 확인한 main JS·CSS·Workbox module·lazy chunk·아이콘을 포함한 현재 URL 20개는 모두 HTTP 200이었다. service worker precache는 22개 entry이며 hash가 바뀌는 asset 이름은 고정해 기록하지 않는다.
- 2026-08-08 최신 운영 검증은 commit `bcd1667dd295b94ba7eb8fef409ae135cee729ee`의 Actions run `31242215654`에서 build job `93064971859`, deploy job `93065018962`와 Pages deployment `5805937150`이 모두 success였다.
- 최신 release는 `nanoid` build dependency 보안 패치 `399be5c02c4c986e805e8436fcaf09fc95ce339e`와 카드 즐겨찾기·공통 필터 정비 `bcd1667dd295b94ba7eb8fef409ae135cee729ee`를 ff-only로 포함한다. release 전 1,129/1,129, TypeScript, production build, PWA, audit 0건과 diff·민감정보 검사가 통과했다. 운영 검증은 읽기 전용 HTTP·bundle 검사이며 운영 앱에서 클릭·입력·storage 조회를 하지 않았다. 이번 release 전에 즐겨찾기·필터는 격리 localhost의 360·390·412·700·1280px, 라이트·다크, 키보드·접근성 이름, 가로 넘침과 console 오류 부재를 확인했지만 Galaxy 실기를 새로 수행하지 않았다. 사용자는 이전 Galaxy HTTPS 운영본 수동 검증에서 첫 문장 공개 자동재생, 전체 답변 연속 재생·일시정지·이어 듣기·문장 선택, 마이크 권한·녹음·녹음 재생과 TTS 상호 정리를 모두 통과했고 이상 현상이 없다고 확인했다.
...
- 답변 익히기 준비의 PC 필터는 정확한 3열 grid를 사용한다. native select 화살표를 숨기고 select·disclosure에 같은 18px 화살표와 우측 위치를 적용하며, 즐겨찾기와 `final_rep`는 본 grid 아래의 같은 빠른 필터 행에 둔다. 700px 이하에서는 기존 1열 필터와 터치 높이를 유지한다.
...
`package.json`의 `test:all`은 다음 27개 스크립트를 순서대로 실행한다. 현재 main의 최신 검증 기준은 1,129/1,129이다.
...
- Pages CI workflow에는 별도 `npm audit` 단계가 없으므로 Actions success와 audit 0건은 별도 증거로 기록한다. 이번 release는 main push 전에 audit 0건, 1,129/1,129, TypeScript, production build, PWA, diff·민감정보 검사를 각각 통과했다.
