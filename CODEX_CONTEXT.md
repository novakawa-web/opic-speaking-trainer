...
>
> 문서 내용의 앱 기능 기준 SHA: `ffe2f9ebc27884b8980f85e06f31a5a31cc41580`
>
...
- 2026-08-09 최신 운영 검증은 commit `ffe2f9ebc27884b8980f85e06f31a5a31cc41580`의 Actions run `31304811808`에서 build job `93223341541`, deploy job `93223388129`과 Pages deployment `5817171097`이 모두 success였다.
- 최신 release는 `nanoid` build dependency 보안 패치 `399be5c02c4c986e805e8436fcaf09fc95ce339e`, 카드 즐겨찾기·공통 필터 정비 `bcd1667dd295b94ba7eb8fef409ae135cee729ee`, 답변 익히기 모바일 필터 압축 `8039f5433ce67e9d763e689999be5de88c9fdc41`, 필터 스크롤 안정화 `e4e6e23d812e7a69817e17558b44439e683e32b2`, 선택형 영어 음성 초안 `e192e0b3d1eb0e9ad1113afe821f52f6d145a7a5`와 음성 초안 UI 잠금·임시 녹음 이탈 경고 `ffe2f9ebc27884b8980f85e06f31a5a31cc41580`를 ff-only로 포함한다. release 전 1,150/1,150, TypeScript, production build, PWA, audit 0건과 diff·민감정보 검사가 통과했다. 운영 검증은 읽기 전용 HTTP·bundle 검사이며 운영 앱에서 클릭·입력·storage 조회를 하지 않았다. 운영 URL, `404.html`, manifest, service worker, 동적으로 찾은 main JS·CSS·lazy chunk·Workbox module·아이콘을 포함한 20개 URL은 모두 HTTP 200이었고 service worker precache는 22개 entry다. 잠금 UI는 격리 localhost의 360·390·412px 세로, 844×390px 가로와 1280px, 라이트·다크에서 음성 초안 조작이 숨겨지고 일반 녹음 조작이 유지되며 가로 넘침과 console 오류가 없음을 확인했다. 실제 마이크·Galaxy 조작은 자동 검증하지 않았고, 사용자의 이전 Galaxy HTTPS 수동 검증에서 일반 마이크 녹음·녹음 재생과 TTS 상호 정리는 통과했다.
...
- 답변 익히기 녹음과 함께 영어를 받아 적는 음성 초안 기반 구현은 남아 있지만, 장시간·Galaxy 신뢰성 문제가 해소될 때까지 main과 운영 Pages에서 사용자 UI를 `SPEECH_DRAFT_FEATURE_ENABLED=false`로 잠근다. 일반 녹음·다시 듣기는 유지하며 임시 녹음 또는 미저장 초안을 잃는 이탈에는 공통 확인 경고를 제공한다.
...
- 답변 익히기 준비의 기타 태그는 여러 개를 선택할 수 있으며 선택한 기타 태그 중 하나 이상을 포함한 카드를 남긴다. 필터 제목은 큰 공통 제목 크기, 선택 항목은 작은 공통 항목 크기를 사용하고 선택된 select·disclosure·`final_rep` 행은 파란색 계열로 강조한다.
- 답변 익히기에서 힌트를 열면 `최소 답변` 바로 아래에 카드 태그를 별도 상자 없이 작은 글씨로 표시한다. `topic_`, `category_`, 기존 오타 호환용 `catecory_` 접두사와 밑줄은 화면 표시에서만 정리하고 원본 tag 값은 바꾸지 않는다.
- 카드 라이브러리 목록, 첫 문장 문제와 답변 익히기 문제에서 같은 카드 ID의 하트 즐겨찾기를 켜고 끌 수 있다. `즐겨찾기만 보기`는 세 카드 선택 화면의 공통 필터이며 다른 덱·태그·상태·`final_rep` 조건과 AND로 결합한다. 하트 button은 `aria-pressed`와 카드별 추가·해제 접근성 이름을 제공하고 학습 문제에서는 44px 조작을 유지한다.
- 답변 익히기 준비의 PC 필터는 정확한 3열 grid를 사용한다. native select 화살표를 숨기고 select·disclosure에 같은 18px 화살표와 우측 위치를 적용하며, 즐겨찾기와 `final_rep`는 본 grid 아래의 같은 빠른 필터 행에 둔다.
- 답변 익히기 준비의 700px 이하 필터와 즐겨찾기·`final_rep` 빠른 필터는 2열로 표시하고 359px 이하에서만 1열로 전환한다. 빠른 필터 행은 불필요한 세로 높이를 만들지 않고 각 조작의 최소 44px 높이를 유지한다.
- 선택지가 많은 다중 선택 필터를 열면 선택 목록 내부가 먼저 스크롤되고 페이지 배경으로 스크롤이 전파되지 않아야 한다. 같은 grid 행의 다른 필터는 열린 목록의 가운데로 이동하지 않고 원래 상단 위치를 유지하며, 이 동작은 세로·가로 화면에 공통으로 적용한다. 이 항목은 새 범용 Skill이 아니라 사용자가 확인한 프로젝트 UX 회귀 규칙으로 유지한다.
- 홈의 학습 카드 요약은 `카드 라이브러리 열기`만 제공한다. 같은 홈의 빠른 시작에 첫 문장 연습 action이 있으므로 요약 카드의 중복 `첫 문장 연습 시작`은 제거했다.
- 카드 라이브러리 20장 단위 표시와 세션 내 필터·스크롤 복원. 카드 grid는 700px 이하 1열, 701px부터 1080px까지 2열, 1081px 이상 3열을 사용한다.
- 카드 라이브러리에서 필터를 바꾸면 표시 개수는 첫 20장으로 다시 계산하지만 현재 세로 스크롤 위치를 보존하고 화면 상단으로 강제 이동하지 않는다.
...
- 첫 문장을 열기 전 `첫 문장 보기`·`정답 확인`은 primary action이고, 공개 후 같은 위치의 `다시 도전`은 secondary action으로 바뀌어 다음 단계 진행과 공개 취소를 구분한다. desktop 최소 폭은 155px이며 700px 이하에서는 기존 2열 action grid, 폭 100%와 최소 높이 48px을 유지한다. `type="button"`, `aria-expanded`와 Space 단축키 계약도 유지한다.
- 700px 이하 첫 문장 일반 연습은 한국어 뜻을 별도 상자 없이 질문 바로 아래에 표시하고, `이전`·`다음`을 공개된 첫 문장 아래에 둔다. 세 상태 button은 56px 터치 높이와 한 줄 배치를 유지하면서 360px에서도 아이콘과 `연습 필요` 문구가 넘치지 않으며, 상태를 고르면 최근 선택 영역 한 곳에서 `상태가 저장되었습니다.`와 실행 취소를 제공한다. 이 영역은 `role="status"` 하나만 사용해 중복 live 안내를 피한다.
...
- 첫 문장 준비와 답변 익히기 준비의 마지막 필터는 각각 별도 localStorage key에 version 1로 저장해 앱을 닫았다 다시 열어도 같은 기기에서 복원한다. 두 화면의 필터는 서로와 카드 라이브러리 필터를 덮지 않으며, dataset에서 사라진 덱·태그와 손상값은 현재 카드 기준의 안전한 기본값으로 정리한다. 카드 라이브러리에서 전달받은 임시 학습 범위에서는 전달 카드만 표시·시작하고 전용 필터를 덮어쓰지 않으며, 전달 범위를 닫으면 저장한 필터로 돌아간다.
- 답변 익히기의 `문제 듣기`·`한국어 뜻 보기`·`카드 수정`은 700px 이하에서도 44px 높이의 3열 한 행으로 표시한다. 전체 답변을 열면 `전체 답변 듣기`와 속도를 모바일 한 행에 두고 그 아래에 `기본 답변`·`나만의 답변` tab을 표시한다. 모바일 문장 button은 번호와 본문을 좁은 2열로 배치해 본문 폭을 확보하며 데스크톱 문장 배치는 유지한다.
- 답변 익히기 전체 답변 문장은 기존보다 약간 큰 1.04rem·굵기 650을 사용한다. 700px 이하에서는 문장 사이 10px, 문단 안 5px, 번호 열 18px, 간격 4px, 좌우 padding 7px로 답변 폭을 확보한다. 선택·재생 강조는 inset shadow라 border 두께로 문장 높이가 바뀌거나 화면이 흔들리지 않는다.
- 답변 익히기 전체 답변의 문장 번호 아래에는 기본 답변·나만의 답변 source별 복습 체크가 있다. 체크는 문장 본문 폭을 줄이지 않는 20px, 모바일 18px 조작이며 `aria-pressed`와 문장 번호별 추가·해제 접근성 이름을 제공한다. 문장 정규화·hash·동일 문장 occurrence로 만든 version 1 ID를 사용하므로 같은 카드·답변 source에서 다시 열었을 때 복원되지만 답변 문장이 바뀌면 일치하지 않는 과거 ID를 표시하지 않는다.
...
- 답변 익히기 전체 답변의 중간 `.answer-learning-answer` wrapper는 별도 border·background·shadow와 padding을 두지 않는다. 바깥 공개 영역 card와 개별 문장 button의 기능 표면은 유지하고, 현재 문장만 `--blue-soft` 배경과 `--blue` 2px inset 테두리로 강조해 중첩된 3중 테두리를 피한다.
...
- 답변 익히기 TTS는 `idle | loading | playing | paused | completed | error` 상태와 `single | continuous` 재생 mode를 분리한다. 전체 답변 듣기는 첫 문장부터 끝까지 연속 재생한다. 정지·완료 상태에서는 문장을 처음 누르면 시간 제한 없는 선택 상태가 되고 같은 문장을 다시 누를 때만 그 문장 하나를 재생한다. 다른 문장을 누르면 선택만 옮긴다. 단일 문장 재생 중 현재 문장을 다시 누르면 정지하고 button 접근성 이름도 `N번 문장 재생 중지`로 바뀐다. 연속 재생 중 문장 터치는 기존처럼 해당 문장부터 끝까지 이어지며 두 번 누르기 대기 상태를 만들지 않는다.
...
- 답변 익히기에서 TTS를 시작하면 녹음을 정리하고, 녹음을 시작하면 TTS를 정지한다. 중지·재생 중인 임시 녹음 또는 미저장 음성 초안이 있을 때 이전·다음·뒤로·홈·답변 source 변경·카드 수정·쉐도잉 전환 전에 사라질 내용을 명시하고 확인받으며, 취소하면 녹음과 현재 화면을 유지한다.
- 녹음 Blob, Object URL, 진행 상태는 현재 컴포넌트 메모리에만 존재한다. localStorage·sessionStorage·AppBackupV1·TSV·Firebase·다운로드 파일에 넣지 않는다.
- 답변 익히기 녹음기의 `녹음하면서 영어 음성 초안 만들기` 구현은 Web Speech API의 `SpeechRecognition` 또는 `webkitSpeechRecognition`을 `en-US`, 연속·중간 결과 mode로 사용한다. 다만 PC 장시간 인식과 Galaxy 브라우저 신뢰성이 확보되지 않아 main과 운영 Pages에서 해당 사용자 UI를 잠근다. 녹음 파일과 다시 듣기 기능은 이 잠금과 무관하게 계속 제공한다.
...
| `opic-favorite-card-ids` | 카드 ID별 즐겨찾기 배열 | 포함, 구버전 누락 시 빈 배열 |
...
| `opic-first-line-filter-preferences` | 첫 문장 준비의 기기별 마지막 필터, version 1 | 제외 |
| `opic-answer-learning-filter-preferences` | 답변 익히기 준비의 기기별 마지막 필터, version 1 | 제외 |
| `opic-answer-learning-sentence-checks` | 카드·답변 source별 문장 복습 체크, version 1 | 제외 |
...
주요 담당 모듈은 `src/utils/cardStorage.ts`, `statusStorage.ts`, `studyStats.ts`, `answerLearningStorage.ts`, `answerLearningSentenceChecks.ts`, `learningFilterPreferences.ts`, `myAnswerStorage.ts`, `cardMemoStorage.ts`, `personalMemoStorage.ts`, `savedPassageStorage.ts`, `cardArchiveStorage.ts`, `cardFavoriteStorage.ts`, `studyPreferences.ts`, `shadowingSettings.ts`다.
...
세션 값은 AppBackupV1, TSV와 클라우드 백업에 포함하지 않는다. 완전히 새 세션에서는 사라져도 되는 UI·진행 데이터이며, 위 두 화면의 마지막 필터만 별도 localStorage preference로 장기 보존한다.

카드 태그 다중 필터는 기존 `opic-navigation-session`과 `opic-answer-learning-session` 객체 안의 `selectedWeeks`, `selectedTopics`, `selectedTypes` 배열을 계속 사용한다. 사용자 화면에서 `selectedWeeks`는 `학습 세트`로 표시하지만 기존 session 호환성을 위해 필드 이름과 session version은 유지한다. 구버전 단일 `selectedTag`가 학습 세트·주제·질문 유형이면 parser가 해당 배열로 승격하고 `final_rep`는 전용 toggle로 옮기며, `v2` 같은 일반 태그는 기타 태그로 보존한다. `favoriteOnly`도 두 session parser와 첫 문장·답변 익히기 전용 localStorage preference에서 boolean으로 정규화하며 구버전 누락은 `false`다. 첫 문장과 답변 익히기의 마지막 필터는 위 두 전용 preference에 저장하지만 JSON·TSV·AppBackup 공개 필드는 추가하지 않는다.
...
- 구버전 v1에서 선택 필드나 `favoriteCardIds`가 빠진 경우 안전한 기본값으로 호환한다. 알 수 없는 필드는 무시하고 위험한 prototype 키를 거부한다.

## 7. 카드 생성·수정·보관과 완전 삭제

- 보관은 카드 본문을 변경하지 않고 `opic-archived-card-ids`만 갱신한다. 학습 기록, 나만의 답변, 메모를 유지한다.
- 완전 삭제는 카드 본문과 해당 카드 ID의 첫 문장 상태·시도, 답변 익히기 상태·시도, 문장 복습 체크, 나만의 답변, 카드 메모, 보관·즐겨찾기 ID 및 현재 학습 세션 참조를 정리한다.
...
- 카드 목록·상세·생성·수정: `src/components/CardList.tsx`, `CardLibrary.tsx`, `CardDetail.tsx`, `CardEditor.tsx`, `FavoriteButton.tsx`, `src/utils/cardCreation.ts`, `cardEditTransaction.ts`, `cardFavoriteStorage.ts`
...
- 답변 익히기와 공통 상태 selector: `src/components/AnswerLearningSetup.tsx`, `AnswerLearning.tsx`, `src/utils/answerLearningStorage.ts`, `answerLearningSentenceChecks.ts`, `answerLearningSession.ts`, `answerLearningSelectors.ts`, `learningFilterPreferences.ts`, `studyStatusOptions.ts`
...
`package.json`의 `test:all`은 다음 27개 스크립트를 순서대로 실행한다. 현재 main의 최신 release 기준은 1,150/1,150이다.
...
| `test:tsv` | 44 |
| `test:answer-learning` | 109 |
| `test:first-line-mock` | 35 |
| `test:card-management` | 35 |
...
- Pages CI workflow에는 별도 `npm audit` 단계가 없으므로 Actions success와 audit 0건은 별도 증거로 기록한다. 이번 release는 main push 전에 audit 0건, 1,150/1,150, TypeScript, production build, PWA, diff·민감정보 검사를 각각 통과했다.
...
npm.cmd audit  # 현재 release SHA의 실제 취약점 수와 exit code를 다시 기록
git diff --check
```
...
- 체크 문장만 모아 보기·듣기와 문제별 질문·첫 문장·체크 문장 복습. 문장별 체크 자체는 같은 기기에서 복원되는 local 기능으로 구현했지만 별도 복습 화면과 기기 간 동기화는 보류한다.

### UX 검토 상태

- 쉐도잉 하단 5버튼 controller의 더 넓은 재설계 조사는 2026-08-03 완료했다. 짧은 가로 화면은 현재의 40px 한 행 구성이 충분한 폭과 터치성을 확보하며, 세로 360~412px 화면까지 속도 선택을 같은 행에 넣으면 일반 버튼 폭이 약 41~51px로 줄어 가독성과 터치성이 나빠지므로 현행 2단 구성을 유지한다. 새 기기 화면이나 구체적인 사용 불편과 측정 근거가 생길 때만 이 범위를 다시 연다.
- 현재 문서에는 바로 구현하도록 승인된 다음 UX 항목이 없다. 새 사용자 관찰이나 별도 조사·계획 승인을 기준으로 다음 작업을 선정한다.
